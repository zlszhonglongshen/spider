...
#         url = "https://www.jianshu.com/search/do?q=" + authorname +"&type=user&page=1&order_by=default"
#         headers = {
...
def getUrl():
    userlink = []
    while len(userlink) == 0:
        # url = raw_input('请输入想查询的作者的链接：')
        url = 'https://www.jianshu.com/u/62478ec15b74'
        pattern = re.compile(u'https:\/\/www\.jianshu\.com\/u\/(.*?$)')
...
def getText(articlelist): #根据文章链接读取文章内容
    ...
    try:
        for url in articlelist:
            ...
            text = soup.find('div',class_="show-content").get_text()
            text = text.replace("\n","")
