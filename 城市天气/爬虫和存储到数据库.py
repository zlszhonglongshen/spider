...
db = psycopg2.connect(database="0000zls", user="mda", password="a", host="172.16.108.101", port="3306")
cur = db.cursor()

# Set up HN story database table schema
cur.execute("DROP TABLE IF EXISTS hn_submissions;")
cur.execute(
    "CREATE TABLE hn_submissions (objectID int PRIMARY KEY, title varchar, url varchar, num_points int, num_comments int, author varchar, created_at timestamp);")
...
while True:
    try:
        ...
        for submission in submissions:
            ...
            title = submission['title'].translate(
                dict.fromkeys([0x201c, 0x201d, 0x2011, 0x2013, 0x2014, 0x2018, 0x2019, 0x2026, 0x2032])).encode('utf-8')
            ...
            created_at = datetime.datetime.fromtimestamp(int(submission['created_at_i']),
                                                         tz=pytz.timezone('America/New_York')).strftime(
                '%Y-%m-%d %H:%M:%S')

            SQL = "INSERT INTO hn_submissions (objectID, title, url, num_points, num_comments, author, created_at) VALUES (%s,%s,%s,%s,%s,%s,%s)"
            insert_data = (
            int(submission['objectID']), title, submission['url'], submission['points'], submission['num_comments'],
            submission['author'], created_at)
