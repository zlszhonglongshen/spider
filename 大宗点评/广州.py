...
DianpingOption = {
    ...
    'locatecityid': 14,
    'categoryid': 10,
    'stop_threshold': 10000,  # if restaurant number in one region go beyond this, stop crawling
    'regionids': [0, 22, 24, 25, 27, 621, 26, 1552, 1554, 1555, 1549, 1964, 1963]
    # 全市 天河 越秀 海珠 白云区 番禺 荔湾 花都 增城 从化 黄埔 萝岗 南沙
 }
...
class DianpingRestaurant(object):
    def __init__(self, id, name, shop_star, branch_name, price_text, category):
        ...
        self._branch_name = branch_name.encode('utf-8')
    ...
    def _analyse_shop_page(self):
        ...
        self._valid = True
        name_soup = soup.find("h1", class_="shop-name")
        ...
        district_soup = soup.find("span", class_=r"og:description")
    ...
    def _analyse_map(self):
        try:
            ...
            for line in lines:
                if r"shopGlat:" in line:
                    lat_line = line.split(":")[1][:-1]
                    self._lat = (lat_line)
                elif r"shopGlng:" in line:
    ...
    #     db_str = [str(self._id), str(self._name), str(self._branch_name), str(self._price_num),
    #               str((float)(self._shop_star) / 10), \
    #               str(self._taste), str(self._surroundings), str(self._service), str(self._category), \
    #               str(self._lng), str(self._lat), str(self._district)]
    #     return db_str
    def get_db_format(self):
...
class DianpingDb(object):
    def __init__(self, tb_name, is_create):
        conn = pymysql.connect(host="localhost", user="root",database="test",charset="gbk")
        self._conn = conn
        ...
        if is_create:
            self._cursor.execute('CREATE TABLE if not exists  ' + tb_name + ' '
                                 + '('
    ...
    def insert_row(self, shop):
        # ON DUPLICATE KEY UPDATE
            self._cursor.execute('INSERT IGNORE INTO ' + self._tb_name + ' (id, name, branch_name, price, ' \
                                 + 'star, taste, surroundings, service, category, longitude, latitude, district) ' \
                                 # + 'values (%d, %s, %s, %d,%f, %f, %f,%f,%s,%f,%f,%s)' \
...
def main():
    ...
    db = DianpingDb('ResTable_guangzhou', region_idx == 0)
