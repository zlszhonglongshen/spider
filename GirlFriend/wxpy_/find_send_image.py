# -*- coding: utf-8 -*-
"""
Created on 2019/1/19 13:57
@Author: Johnson
@Email:593956670@qq.com
@File: find_send_image.py
"""
# -*- coding: utf-8 -*-
"""
...
def RandomSelectPic(fileDir):
    '''
    :param fileDir: 原始图片路径
    :return: 随机挑选出的图片名称以及路径
    '''
    pathDir = os.listdir(fileDir) #原始路径
    sample = random.sample(pathDir,1)[0] #随机挑选出一张图片
    return fileDir+sample


#计算相恋天数
inLoveDate = datetime(2018, 10,27)
...
def job():
    for i in userName:
        ic.send(msg=message, toUserName=i)
        ic.send_image(str(RandomSelectPic(imageDir)), toUserName=i)
    time.sleep(10)
        # t = Timer(30,job)
        # t.start()
