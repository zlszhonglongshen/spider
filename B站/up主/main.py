#coding:utf-8
import requests,pymysql,re,os,traceback
...
#建表
try:
    cursor.execute('create table author_db(uid varchar(32) not null primary key, author text, sex text, sign text, fansNumber text, watchNumber text, registerTime text, birthday text, address text, icon text, link text)')
    conn.commit()
    cursor.execute('create table video_db(aid varchar(32) not null primary key, videoName text, watchNumber text, rank text, coinNumber text, collectNumber text, replyNumber text, uid varchar(32), icon text, link text)')
    conn.commit()
except:
    pass
